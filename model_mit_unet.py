...
from segformer.segformer_head import *
from segformer.mix_transformer import *
import copy
...
class Net(nn.Module):
    ...
    def __init__(self,
        ...
        self.encoders_mit = torch.nn.ModuleList()
        self.encoders_mit.append(encoder())
        if encoder_ckpt is not None:
            checkpoint = torch.load(encoder_ckpt, map_location=lambda storage, loc: storage)
            # import ipdb;ipdb.set_trace()
            self.encoders_mit[0].load_state_dict({k.replace('backbone.', ''):v for k, v in checkpoint['state_dict'].items()}, strict=False)
        self.encoders_mit.append(copy.deepcopy(self.encoders_mit[0]))
        encoder_dim = self.encoders_mit[0].embed_dims
        # [64, 128, 320, 512]
    ...
    def forward(self, batch):
        ...
        # import ipdb;ipdb.set_trace()
        encoder = self.encoders_mit[organs[0].item() // 4](x)
