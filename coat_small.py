...
class CoaT(nn.Module):
    """ CoaT class. """
    def __init__(self, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[0, 0, 0, 0, 0], 
                 serial_depths=[0, 0, 0, 0, 0], parallel_depth=0,
                 num_heads=0, mlp_ratios=[0, 0, 0, 0, 0], qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
...
def coat_small(**kwargs):
    model = CoaT(return_interm_layers=True, patch_size=4, embed_dims=[152, 320, 320, 320, 320], serial_depths=[2, 2, 2, 2, 2], parallel_depth=6, num_heads=8, mlp_ratios=[4, 4, 4, 4, 4], **kwargs)
    model.default_cfg = _cfg_coat()
    return model
